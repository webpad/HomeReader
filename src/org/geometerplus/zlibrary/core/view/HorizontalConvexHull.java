...
public final class HorizontalConvexHull implements Hull {
	...
	public void draw(ZLPaintContext context, int mode) {
		...
		while (!rectangles.isEmpty()) {
			...
			Rect r = iter.next();
			x = r.right + 2;
			xList.add(x); yList.add(r.top+20);
			while (iter.hasNext()) {
				xPrev = x;
				r = iter.next();
				x = r.right + 2;
				if (x != xPrev) {
					final int y = (x < xPrev) ? r.top + 20 : r.top+5;
					xList.add(xPrev); yList.add(y);
					xList.add(x); yList.add(y);
				}
//				context.drawLine(r.left, r.bottom,r.right, r.bottom);
			}
			xList.add(x); yList.add(r.bottom + 20);
			...
			while (iter.hasPrevious()) {
				...
				x = r.left - 2;
				if (x != xPrev) {
					final int y = (x > xPrev) ? r.bottom : r.bottom + 20;
					xList.add(xPrev); yList.add(y);
					xList.add(x); yList.add(y);
				}
//				context.drawLine(r.left, r.bottom,r.right, r.bottom);
