...
public abstract class ZLTextView extends ZLTextViewBase {
	...
	private void drawSelectionCursor(ZLPaintContext context, ZLTextPage page, SelectionCursor.Which which) {
		...
		if (pt != null) {
			// draw text Select cursor shift up
			int shift = getTextStyleCollection().getBaseStyle().getFontSize()/2;
			SelectionCursor.draw(context, which, pt.X, pt.Y + shift, getSelectionBackgroundColor());
		}
	}
	...
	private synchronized int computeTextPageNumber(int textSize) {
	    ...
	    //only for debug
	    /*
	    int paragraphIndex = myModel.findParagraphByTextLength(sizeOfFullText());
	    ALog.debug("paragraphIndex:"+paragraphIndex);
	    for (int index=0;index<paragraphIndex;index++) {
	        ALog.debug("myModel.getTextLength("+index+"):"+myModel.getTextLength(index));
	    }
	    */
	    ///////////////////////////////////////
		...
		final float pages = textSize * factor;
                  pageNumber = Math.max((int)(pages + 1.0f - 0.5f * factor), 1);

		///////////////////////////////////////
		//support all png epub file
	         int indexPerParagraph = 3;
		if(factor<0)
		{
	            pageNumber = 1+(textSize/indexPerParagraph);
		}
		///////////////////////////////////////
...
///////////////////////////////////////
//support all png epub file
int firstParagraphIndex = 6;
int indexPerParagraph=9;
if((paragraphIndex==0)&&(wordIndex==0))
{
    paragraphIndex = firstParagraphIndex+(page-1)*indexPerParagraph; 
}
///////////////////////////////////////
	...
	private void drawTextLine(ZLTextPage page, List<ZLTextHighlighting> hilites, ZLTextLineInfo info, int from, int to) {
		...
		for (int wordIndex = info.RealStartElementIndex; wordIndex != endElementIndex && index < to; ++wordIndex, charIndex = 0) {
			...
			if (element == area.Element) {
				...
				// draw text frame outline shift up
				int shift = getTextStyleCollection().getBaseStyle().getFontSize()/2;
				
				final int areaX = area.XStart;
				final int areaY = area.YEnd - getElementDescent(element) - getTextStyle().getVerticalAlign(metrics()) + shift - 4;
