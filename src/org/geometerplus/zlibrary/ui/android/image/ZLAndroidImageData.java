					...
					switch (scaling) {
						...
						case FitMaximum:
						{
							...
							if (bWidth > 0 && bHeight > 0 &&
								...
								final Bitmap scaled =
									Bitmap.createScaledBitmap(myBitmap, w, h, true);
								if (scaled != null) {
									myBitmap = scaled;
								}
							}
						...
						case IntegerCoefficient:
						{
							...
							if (bWidth>Globals.fullScreenWidth/2 && bHeight>Globals.fullScreenHeight/2){
								float widthScale=Globals.fullScreenWidth/(float)bWidth;
								float heightScale=Globals.fullScreenHeight/(float)bHeight;

								if ((int)(heightScale*bWidth)>Globals.fullScreenWidth){
									myBitmap=Bitmap.createScaledBitmap(myBitmap,(int)Globals.fullScreenWidth, (int)(widthScale*bHeight), true);

								}else{
									myBitmap=Bitmap.createScaledBitmap(myBitmap,(int)(heightScale*bWidth), (int)Globals.fullScreenHeight, true);
								}
							}
