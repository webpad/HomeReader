...
public class ZoomImageView extends View {
	// Canvas _canvas;
	private final int STATUS_INIT = 1;	// 初始化状态常量
	private final int STATUS_ZOOM_OUT = 2;// 图片放大状态常量
	private final int STATUS_ZOOM_IN = 3; // 图片缩小状态常量
	public final int STATUS_MOVE = 4;	// 图片拖动状态常量

	private Matrix matrix = new Matrix();// 用于对图片进行移动和缩放变换的矩阵
	private Bitmap sourceBitmap;	// 待展示的Bitmap对象
	public int currentStatus; 		// 记录当前操作的状态，可选值为STATUS_INIT、STATUS_ZOOM_OUT、STATUS_ZOOM_IN和STATUS_MOVE
	...
	public float lastXMove = -1;	// 记录上次手指移动时的横坐标
	public float lastYMove = -1; 	// 记录上次手指移动时的纵坐标
	...
	public boolean isInitRatio=true;
	/**
		...
		case MotionEvent.ACTION_MOVE:
			if (event.getPointerCount() == 1) {
				...
				if (fingerDis > lastFingerDis) {
					...
					isInitRatio=false; // do not slide page
				...
				if ((currentStatus == STATUS_ZOOM_OUT && totalRatio < 4 * initRatio)
					...
					if (totalRatio > 4 * initRatio) {
						...
						isInitRatio=true; // can slide page
					}
					// 调用onDraw()方法绘制图片
					invalidate();
...
//			break;
	 ...
	 */
	private static ZoomImageView imageview;

	public void initZoom(ZoomImageView iv) {
		// TODO
		currentStatus = STATUS_INIT;

		totalTranslateX = 0; // 记录图片在矩阵上的横向偏移值
		totalTranslateY = 0; // 记录图片在矩阵上的纵向偏移值
	...
	public static boolean isRefreshEnd = true;
	public static int refreshCountLimit = 1; // Set second times to full refresh
												// screen
	public static int refreshCount = refreshCountLimit;

	public static Handler moveTime = new Handler();
	public static Runnable moveRun = new Runnable() {
		public void run() {
			if (refreshCount <= 0) {
				imageview.postInvalidate(RefreshClass.UPDATE_MODE_SCREEN); // Full Screen Refresh
				isRefreshEnd = true;
	...
	public void refreshScreen(int count) {
