...
class CoverHolder {
	...
	private FBTree fbTree;
	synchronized void setFBTree(FBTree tree){
		this.fbTree = tree;
	}
				...
				*/
				...
				myManager.runOnUiThread(new Runnable() {
					...
					public void run() {
						synchronized (CoverHolder.this) {
							if (Key.equals(myKey)) {
								CoverView.setBackgroundDrawable(new BitmapDrawable(CoverView.getResources(), coverBitmap));
								CoverView.setImageBitmap(BitmapFactory.decodeResource(CoverView.getResources(), Globals.getCoverTypeTagResourceId(fbTree.getName())));
							}
						}
					}
				});
