...
public final class FBReader extends FBReaderMainActivity implements ZLApplicationWindow, SensorEventListener {
	...
	public static boolean showBookmarks(Activity activity){
		BookmarksActivity.switchToAllBook();

		if (myFBReaderApp == null) {

			Book book = new Book(0, Globals.PATH_SDCARD+"/"+Globals.ASSET_BOOKMARK_FILE, "", "", "");			
			try{
				final Intent intent = defaultIntent(activity);
	...
	private static FBReaderApp myFBReaderApp;
	...
	private static int mWifiState = 4;

	private FooterPopupWindow mFooterPopupWindow;

	private SensorManager sensorManager;
	private Sensor accelerometer;
	private float mSensorX;
	private float mSensorY;
	private float filter_val = 5f;
	private boolean has_accelerometer = false;
	private int direction_up = 0;
	private int direction_down = 1;
	private int direction_left = 2;
	private int direction_right = 3;
	...
	private synchronized void openBook(Intent intent, final Runnable action, boolean force) {
		...
		if (myBook != null) {
			...
			if (!file.exists()) {
				...
				// back to LibraryActivity page, when path crash; don't show any
				// alert dialog

				// if (file.getPhysicalFile() != null) {
				// file = file.getPhysicalFile();
				// }
				// UIMessageUtil.showErrorMessage(this, "fileNotFound",
				// file.getPath());
				myBook = null;
			} else {
		...
		Config.Instance().runOnConnect(new Runnable() {
			...
			public void run() {
				if (myBook == null) {
					// back to LibraryActivity page, when path crash; delay 3sec
					// to rebuild internal sdcard path.
					try {
						Thread.sleep(3000);
	...
	protected void onCreate(Bundle icicle) {
		...
		if (myFBReaderApp == null) {
			 ...
			 myFBReaderApp = new FBReaderApp(Paths.systemInfo(this), new
		...
		if (myFBReaderApp.getPopupById(FooterFontSizePopup.ID) == null) {
			new FooterFontSizePopup(myFBReaderApp);
		}
		...
		// myFBReaderApp.addAction(ActionCode.SET_SCREEN_ORIENTATION_SYSTEM, new
		// SetScreenOrientationAction(this, myFBReaderApp,
		// ZLibrary.SCREEN_ORIENTATION_SYSTEM));
		// myFBReaderApp.addAction(ActionCode.SET_SCREEN_ORIENTATION_SENSOR, new
		// SetScreenOrientationAction(this, myFBReaderApp,
		...
		Globals.fullScreenWidth = (int) (displaymetrics.widthPixels * 0.93);
		Globals.fullScreenHeight = (int) (displaymetrics.heightPixels * 0.93);

		tvBookProgress = (TextView) findViewById(R.id.tv_book_progress);
		sbBookProgress = (SeekBar) findViewById(R.id.sb_book_progress);

		sbBookProgress.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
			...
			public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
				if (fromUser) {
					tvBookProgress.setText(progress + " / " + myFBReaderApp.BookTextView.pagePosition().Total);
				}
			}
		});

		myFBReaderApp.BookTextView.setToView(tvBookProgress, sbBookProgress);

		sensorManager = (SensorManager) getSystemService(Context.SENSOR_SERVICE);
		accelerometer = sensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
		has_accelerometer = sensorManager.registerListener(this, accelerometer, SensorManager.SENSOR_DELAY_GAME);
	}
	...
	protected void onStart() {
		...
		((FooterFontSizePopup) myFBReaderApp.getPopupById(FooterFontSizePopup.ID)).setPanelInfo(this, myRootView);
	...
	protected void onResume() {
		...
		Globals.closeWaitDialog(this, 200);
		myStartTimer = true;
		Config.Instance().runOnConnect(new Runnable() {
			@Override
			public void run() {
				...
				try {
					brightnessLevel = (int) (((android.provider.Settings.System.getInt(getContentResolver(),
							android.provider.Settings.System.SCREEN_BRIGHTNESS)) / 255f) * 100);
		...
		myMainView.checkRefreshFrequency(); // Jacky 20160615 ,when set
											// Reference pageRefresh , need
											// re-set
											// myMainView.checkRefreshFrequency()
	...
	protected void onPause() {
		...
		try {
			...
			// do nothing, this exception means that myBatteryInfoReceiver was
			// not registered
		}
	...
	public void navigate() {
		((FooterFontSizePopup) myFBReaderApp.getPopupById(FooterFontSizePopup.ID)).runNavigation();
	}

	public boolean navigateClose() {
		return ((FooterFontSizePopup) myFBReaderApp.getPopupById(FooterFontSizePopup.ID)).closeNavigation();
	}

	public void showNavigationFontSize() {
		if (!myFBReaderApp.getTextView().imageOnlyCheck()) {// check image page
			((FooterFontSizePopup) myFBReaderApp.getPopupById(FooterFontSizePopup.ID)).runNavigation();
		}
	}

	public void changeNavigationFontSizeProgress(int value) {
		((FooterFontSizePopup) myFBReaderApp.getPopupById(FooterFontSizePopup.ID)).setUpFontSizeSliderValue(value);
	}

	public void setFontSize() {
		((FooterFontSizePopup) myFBReaderApp.getPopupById(FooterFontSizePopup.ID)).setFontSize();
	}

	/*
	 * private Menu addSubmenu(Menu menu, String id) { return
	 * menu.addSubMenu(ZLResource.resource("menu").getResource(id).getValue());
	 * }
	 * 
	 * private void addMenuItem(Menu menu, String actionId, Integer iconId,
	 * String name) { if (name == null) { name =
	 * ZLResource.resource("menu").getResource(actionId).getValue(); } final
	 * MenuItem menuItem = menu.add(name); if (iconId != null) {
	 ...
	 * 
	 * private void addMenuItem(Menu menu, String actionId) { addMenuItem(menu,
	 * actionId, null, null); }
	 * 
	 * private void fillMenu(Menu menu, List<MenuNode> nodes) { for (MenuNode n
	 * : nodes) { if (n instanceof MenuNode.Item) { final Integer iconId =
	 * ((MenuNode.Item)n).IconId; if (iconId != null) { addMenuItem(menu,
	 * n.Code, iconId); } else { addMenuItem(menu, n.Code); } } else // if (n
	 * instanceof MenuNode.Submenu) { final Menu submenu = addSubmenu(menu,
	 * n.Code); fillMenu(submenu, ((MenuNode.Submenu)n).Children); } } }
	 * 
	 * private void setupMenu(Menu menu) { final String menuLanguage =
	 * ZLResource.getLanguageOption().getValue(); if
	 * (menuLanguage.equals(myMenuLanguage)) { return; } myMenuLanguage =
	 * menuLanguage;
	 * 
	 * menu.clear(); fillMenu(menu, MenuData.topLevelNodes()); synchronized
	 * (myPluginActions) { int index = 0; for (PluginApi.ActionInfo info :
	 * myPluginActions) { if (info instanceof PluginApi.MenuActionInfo) {
	...
	public void showPopUpMenu() {
		if (mFooterPopupWindow == null) {
			mFooterPopupWindow = new FooterPopupWindow(myFBReaderApp.mActivity, has_accelerometer, new FooterPopupWindow.Callback() {
...
//					myFBReaderApp.runAction(ActionCode.SEARCH);
				...
				public void onRotate() {
					if (!has_accelerometer) {
						Log.d("Dylan", "isComing");
						if (getZLibrary().getOrientationOption().getValue()
								.equals(ZLibrary.SCREEN_ORIENTATION_PORTRAIT)) {
	...
	public void onSensorChanged(SensorEvent event) {
		...
		if (mSensorY > filter_val) {
			// UP
			if (!getZLibrary().getOrientationOption().getValue().equals(ZLibrary.SCREEN_ORIENTATION_PORTRAIT)) {
				rotateReader(direction_up);
			}
			return;
		}
		if (mSensorY < -filter_val) {
			// DOWN
			if (!getZLibrary().getOrientationOption().getValue().equals(ZLibrary.SCREEN_ORIENTATION_REVERSE_PORTRAIT)) {
				rotateReader(direction_down);
			}
			return;
		}
		if (mSensorX < -filter_val) {
			// LEFT
			if (!getZLibrary().getOrientationOption().getValue()
					.equals(ZLibrary.SCREEN_ORIENTATION_REVERSE_LANDSCAPE)) {
				rotateReader(direction_left);
			}
			return;
		}
		if (mSensorX > filter_val) {
			// RIGHT
			if (!getZLibrary().getOrientationOption().getValue().equals(ZLibrary.SCREEN_ORIENTATION_LANDSCAPE)) {
				rotateReader(direction_right);
			}
			return;
		}
	}
	...
	public void rotateReader(int direction) {
		if (direction == direction_up) {
			myFBReaderApp.runAction(ActionCode.SET_SCREEN_ORIENTATION_PORTRAIT);
		} else if (direction == direction_down) {
			myFBReaderApp.runAction(ActionCode.SET_SCREEN_ORIENTATION_REVERSE_PORTRAIT);
		} else if (direction == direction_left) {
			myFBReaderApp.runAction(ActionCode.SET_SCREEN_ORIENTATION_REVERSE_LANDSCAPE);
	...
	/**
	 * for select text cursor fast
	 * @param isForNtxAppsOnly
	 */
