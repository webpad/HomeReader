...
//			String fileName = file.getPath().substring(file.getPath().lastIndexOf("/") + 1, file.getPath().length());
			...
			if (file.isDirectory()){
				set.add(file);
			}else if (file.isArchive() || Collection.getBookByFile(file.getPath()) != null){
				
				final int index = file.getPath().lastIndexOf('.');
				final String myExtension = ((index > 0) ? file.getPath().substring(index).toLowerCase().intern() : "");				
				
				for (String fileType : Globals.searchBookType){
					if (myExtension.equals(fileType) && file.getPath().substring(0,1).equals(".")==false){
...
//				for (String fileType : Globals.searchBookType){
//					if ((fileName.toLowerCase().indexOf(fileType) > -1 && fileName.substring(0,1).equals(".")==false)){
//						set.add(file);
//						break;
//					}
//				}
				...
				final int index = file.getShortName().lastIndexOf('.');
				final String myExtension = ((index > 0) ? file.getShortName().substring(index).toLowerCase().intern() : "");				
				
				for (String fileType : Globals.searchImageType){
					if (myExtension.equals(fileType) && file.getShortName().substring(0,1).equals(".")==false){
						// add matched path into array
						set.add(file);
						break;
					}
...
//					if ((fileName.toLowerCase().indexOf(type) > -1 && fileName.substring(0,1).equals(".")==false)){
//            			set.add(file);
//            			break;
//            		}
//            	}
