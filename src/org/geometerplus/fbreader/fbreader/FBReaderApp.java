...
import org.geometerplus.android.fbreader.OpenBookAsyncTask;
...
public final class FBReaderApp extends ZLApplication {
	...
	public boolean isSameBookEncryption = false;
	...
	public void openBook(Book book, final Bookmark bookmark, final Runnable postAction, Notifier notifier) {
		if (Model != null) {
			if (book == null || bookmark == null && Collection.sameBook(book, Model.Book)) {
				if (isSameBookEncryption){
					new OpenBookAsyncTask(mActivity, this, book, bookmark, isSameBookEncryption).execute();
				}

				if (NativeFormatPlugin.isReload()){
					if (bookmark != null) {
						gotoBookmark(bookmark, false);
					}
					new OpenBookAsyncTask(mActivity, this, bookToOpen, bookmark, false).execute();
				}
	...
	public synchronized void openBookInternal(final Book book, Bookmark bookmark, boolean force) {
		if (!force && Model != null && Collection.sameBook(book, Model.Book)) {
			if (bookmark != null) {
				gotoBookmark(bookmark, false);
			}
			if (false == NativeFormatPlugin.isReload()){
				return;
			}
		...
		try {
			...
			if (false == book.getPath().equals(Globals.PATH_SDCARD+"/"+Globals.ASSET_BOOKMARK_FILE)){
