...
//		final int horMargin = Math.min(dpi / 5, Math.min(x, y) / 30);
		final int horMargin = Math.min((int)(dpi / 4.5f), Math.min(x, y) / 27); // Jacky 20160426 modify Margins space
...
//			new ZLIntegerRangeOption("Options", "TopMargin", 0, 100, 0);
			new ZLIntegerRangeOption("Options", "TopMargin", 0, 100, horMargin); // Jacky 20160426 modify Margins space of Top

		BottomMargin =
			new ZLIntegerRangeOption("Options", "BottomMargin", 0, 100, 4+10); // +10 ==> Check ZLAndroidWidget.java ( footerOffset = -10 )
		SpaceBetweenColumns =
		...
		FooterHeight =
			new ZLIntegerRangeOption("Options", "FooterHeight", 12, dpi / 3, (dpi / 20)*3); // Jacky 20160702 set Font & Bar Size
		ColorProfileName =
