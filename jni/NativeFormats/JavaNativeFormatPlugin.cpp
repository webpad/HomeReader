...
extern "C"
JNIEXPORT jint JNICALL Java_org_geometerplus_fbreader_formats_NativeFormatPlugin_readingProgress(JNIEnv* env, jobject thiz, jobject javaModel, jstring javaCacheDir, bool isRunning) {
	shared_ptr<FormatPlugin> plugin = findCppPlugin(thiz);
	const std::string cacheDir = AndroidUtil::fromJavaString(env, javaCacheDir);
		...
		return plugin->readingPluginProgress(*model, isRunning);
}
extern "C"
